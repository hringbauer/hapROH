# run hapROH using maf5 1000G ref panel
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', action="store", dest='con', type=float, required=True,
                        help="contamination rate used when inferring ROH blocks")
    ...
    if not os.path.exists(f'{basepath}/hdf5_maf5'):
        os.makedirs(f'{basepath}/hdf5_maf5')
    print(f'saving hdf5 files in {basepath}/hdf5_maf5')

    t1 = time.time()
    prms = [ [mpileup_path + f'{iid}.chr{ch}.mpileup', 
            f'/mnt/archgen/users/yilei/Data/1000G/1000g1240khdf5/all1240/maf5_auto/maf5_chr{ch}.hdf5',
            iid, -np.inf, np.inf, f'{basepath}/hdf5_maf5', False] for ch in range(1, 23)]
    ...
    hapsb_ind(iid, chs=range(1,23), 
        ...
        c=args.con, roh_min_l_final=0.04, roh_in=1, roh_out=20, roh_jump=300, e_rate=err, e_rate_ref=1e-3, 

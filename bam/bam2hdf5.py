...
if __name__ == '__main__':
    ...
    mle1, low95, up95 = hapCon_chrom_BFGS(iid, 'X', save=False, save_fp=False, n_ref=2504, diploid_ref=True, 
    ...
    with open(f'{iid}.hapcon.trim5.txt', 'w') as out:
        ...
        out.write(f'Method1: BFGS on contamination rate\n')
        out.write(f'\tEstimated genotyping error via flanking region: {round(err, 6)}\n')
        out.write(f'\tMLE for contamination: {round(mle1[0], 6)} ({round(low95[0], 6)} - {round(up95[0], 6)})\n')
        out.write("\n")

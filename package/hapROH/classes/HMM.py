...
from .genomicData import DataType, GenomicData
...
def _forward_backward(
    ...
    for i in range(nb_snp - 1):
        sum_roh = (
            1 - post_pb[0, i]
        )  # = post_pb[1:].sum(axis=0) because of normalisation
...
class HMM:
    ...
    def __init__(
        self,
        sample_data: GenomicData,
        ref_data: GenomicData,
        r_map: np.ndarray,
        r_in: float,
        r_out: float,
        r_jump: float,
        ...
        if ref_data.datatype != DataType.GT:
            raise ValueError(
                f"Expecte reference pannel to contain GT, but contains datatype {ref_data.datatype}"
            )
        self.ref_panel = ref_data.data.reshape(
            ref_data.data.shape[0], -1
        )  # (nb_snp, nb_samples, 2) -> (nb_snp, 2*nb_samples)
        ...
        self.proba_t = get_transi_proba(
            r_in, r_out, r_jump, self.ref_panel.shape[1], r_map
        )  # shape (5, nb_snp-1)
        print_memory_usage(logger)

        logger.debug("Computing emission probabilities")
        allele_freq = self.ref_panel.mean(axis=1)
        self.proba_e = get_emi_proba(
            sample_data, allele_freq, error_rate
        )  # shape (3, nb_snp, nb_samples)
        print_memory_usage(logger)

    def calc_posterior_proba(
        self, backend: Literal["python", "numba", "cython"] = "cython"
    ) -> np.ndarray:
        """
        Compute the posterior probability of each state at each locus.
        Args:
        ...
        match backend:
            case "numba":
                return _forward_backward_numba(
                    ref_panel, proba_e, proba_t, nb_ref, nb_snp, nb_samples
                )
            case "cython":
                return _forward_backward_cython(
                    ref_panel, proba_e, proba_t, nb_ref, nb_snp, nb_samples
                )
            case "python":
        ...
        raise ValueError(
            f"Unknown backend ({backend}). Available implementations are: 'python', 'numba' or 'cython'"
        )

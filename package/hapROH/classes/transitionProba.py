...
def get_transi_proba(r_in: float, r_out: float, r_jump: float, n_ref: int, r_map: np.ndarray) -> TransitionProba:
    """Analytical exponentiation of the transition rate matrix."""
    # analytic formula for the exponentiaton of the transition rate
    S = r_in + r_out
    P1 = np.array([r_out, r_in / n_ref, r_out, r_in / n_ref, r_in / n_ref]) / S       # coeff 1
    P2 = np.array([r_in, -r_in / n_ref, -r_out, r_out / n_ref, r_out / n_ref]) / S    # coeff exp(-Sr)
    P3 = np.array([0, 0, 0, (n_ref - 1), -1]) / n_ref                                 # coeff exp(-(r_out+r_jump)r)

    return TransitionProba(P1[:, None]
                         + P2[:, None] * np.exp(-S*r_map)
                         + P3[:, None] * np.exp(-(r_out+r_jump)*r_map))

def _get_transi_proba_slow(r_in: float, r_out: float, r_jump: float, n_ref: int, r_map: np.ndarray) -> TransitionProba:
    ...
    return TransitionProba(np.array([T_exp[0, 0], T_exp[0, 1], T_exp[1, 0], T_exp[1, 1], T_exp[1, 2] / (n_ref - 1)]))

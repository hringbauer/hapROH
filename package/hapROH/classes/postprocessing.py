...
def call_roh(
    ...
    values = {
        ...
        "StartBP": df_snp["pos"].iloc[starts].values,
        "EndBP": df_snp["pos"].iloc[ends].values,
        "StartM": df_snp["map"].iloc[starts].values,
        "EndM": df_snp["map"].iloc[ends].values,
    }
...
def merge_blocks(
    df_roh: pd.DataFrame, max_gap=0.005, min_len1=0.04, min_len2=0.02
) -> pd.DataFrame:

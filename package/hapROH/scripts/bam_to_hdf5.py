...
def main() -> None:
    ...
    parser.add_argument(
        "-i", "--path-bam", required=True, type=str, help="Path to the input bam file."
    )
    parser.add_argument(
        "-r",
        "--path-ref",
    ...
    parser.add_argument(
        "-s", "--sample-name", required=True, type=str, help="Name to use in the hdf5."
    )
    parser.add_argument(
        "-c",
        "--chrom",
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing bam files."
    )
    parser.add_argument(
        "--samtools",
        dest="path_samtools",
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not run samtools using command '{samtools_cmd} --version': {e}. "

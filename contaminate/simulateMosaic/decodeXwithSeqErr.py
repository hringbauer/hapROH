...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        _, conMLE, lower95, upper95 = hapCon_chrom(iid, ch='X', 
            path_targets=f"{outFolder}/err{err}data.h5",
            ...
            c=np.arange(0.15, 0.4, 0.005), roh_in=1, roh_out=0, roh_jump=300, e_rate=err_rate, e_rate_ref=e_rate_ref, 
    ...
    with open(f'{outFolder}/batchresults.txt', 'w') as out:
        out.write(f'###contamination=0.075, coverage={cov}, genotyping error index={err}, genotyping error used in inference=1e-2, ref err=1e-3\n')

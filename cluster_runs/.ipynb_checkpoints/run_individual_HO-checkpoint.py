...
def analyze_individual_ho(iid, chs=range(1, 23), n_ref=2504, save=True, save_fp=False, exclude_pops=[],
    ...
    for prm in prms:
        ...
        analyze_chromosome_gt(*prm)

    # Merge results for that Individual
    combine_individual_data(base_out_folder, iid=iid, delete=delete, chs=chs)
    if output == True:
        print(f"Run for Individual {iid}: Complete! Yeah!!")
    return


#########################################################

def get_iid_from_i(meta_df, i):
    """Get the Individual IID"""
    ho_df = meta_df[meta_df["study"]=="Lazaridis et al. 2014"] # Extract HO individuals
    if i<0 or i>=len(ho_df):    # Sanity Check
        raise RuntimeError(f"Index {i} out of Range of HO Meta.") 
    iid = ho_df["iid"].values[i]
    return iid
...
if __name__ == "__main__":
    ...
    analyze_individual_ho(iid=iid, chs=range(1, 23), n_ref=2504, save=True, save_fp=False, exclude_pops=[],
                          base_out_folder="./Empirical/HO/", prefix_out="",
                          roh_in=100, roh_out=100, roh_jump=300, e_rate=0.001,
